...
(function () {
  ...
  function V(a, b) {
    ...
    D = D ? W(D, v) : this.h;
    E
      ? ((l || z).appendChild(this.body), (n = Math.max(Math.min(this.body.clientWidth + 2 * O + 1, u), C)), (q = Math.max(Math.min(this.body.clientHeight + this.h + O + 1, v), D)), this.g.appendChild(this.body))
  ...
  function la() {
    ...
    e = 0;
    for (var g, l; e < a; e++)
      (d = x[e]), (g = (d.left || d.right) + ":" + (d.top || d.bottom)), (l = Math.min((P - d.left - d.right) / c[g], 250)), d.resize((l + 1) | 0, d.h, !0).move((d.left + b[g] * l) | 0, Q - d.bottom - d.h, !0), b[g]++;
  }
  function X(a, b) {
    function c(h) {
      r(h);
